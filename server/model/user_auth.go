...
type AuthUnit struct {
	// Attribute
	BelongSection int `json:"section"` // 权限单元所属板块：[0]不属于任何板块；[n]表示对应板块
	// Auth
	ReadLv   int `json:"read_lv"`   // 帖子阅读等级：[n]表示阅读权重，仅能阅读权限等级不高于该项的帖子，若为 0 表示部门所有帖子都可以查看，若为 -1 表示所有帖子都可以查看
	ManageLv int `json:"manage_lv"` // 帖子管理等级：[0]无法管理帖子；[1]能够管理自己帖子（不包括删除）；[2]能够管理对应板块所有帖子（包括删除）；[3]能够管理所有帖子；
}
