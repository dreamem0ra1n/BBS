    ...
    <div class="container main-container left-main size-320">
      <div class="left-container">
        <div class="main-content">
          <!--<search-topics-nav :nodes="nodes" :old="true" />-->
          <div v-if="searchPage && searchPage.results">
            <search-topic-list :search-page="searchPage" :old="true" />
            <pagination
              :page="searchPage.page"
              :url-prefix="'/search/old?q=' + keyword + '&p='"
            />
          </div>
...
export default {
  async asyncData({ $axios, query, store }) {
    ...
    const nodes = json.map((forum) => {
      return { nodeId: forum.fid, name: forum.name }
    })
  ...
  methods: {
    async searchTopic() {
      await this.$store.dispatch('search/searchOld')
    },
  },
}
</script>
